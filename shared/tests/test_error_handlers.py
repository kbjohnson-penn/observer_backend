...
from shared.api.error_handlers import (
    ObserverAPIException, ObserverNotFound, ObserverPermissionDenied,
    ObserverValidationError, handle_not_found, handle_permission_denied,
    handle_validation_error, handle_server_error, safe_get_object_or_404,
    ErrorHandlerMixin
)
...
class ErrorHandlerMixinTest(APITestCase):
    ...
    def test_handle_exception_unexpected(self, mock_logger):
        ...
        mock_logger.error.assert_called_once_with(
            "Unhandled exception in TestViewSet",
            exc_info=exc
        )
