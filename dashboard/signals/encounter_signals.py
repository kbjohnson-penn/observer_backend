...
from ..models.encounter_models import Encounter, EncounterSource, Department, EncounterSimCenter, EncounterRIAS, EncounterFile, MultiModalDataPath
from ..graph_models import EncounterNode, EncounterSourceNode, DepartmentNode, EncounterSimCenterNode, EncounterRIASNode, PatientNode, ProviderNode

@receiver(post_save, sender=EncounterSource)
def create_or_update_encounter_source(sender, instance, created, **kwargs):
    encounter_source_nodes = EncounterSourceNode.nodes.filter(django_id=instance.id)
    if created:
        if not encounter_source_nodes:
            EncounterSourceNode(name=instance.name, django_id=instance.id).save()
    else:
...
def delete_encounter_source(sender, instance, **kwargs):
    try:
        encounter_source_node = EncounterSourceNode.nodes.get(django_id=instance.id)
...
def create_or_update_encounter_node(sender, instance, created, **kwargs):
    patient = PatientNode.nodes.get_or_none(django_id=instance.patient.id) if instance.patient else None
    provider = ProviderNode.nodes.get_or_none(django_id=instance.provider.id) if instance.provider else None
    department = DepartmentNode.nodes.get_or_none(django_id=instance.department.id) if instance.department else None
    encounter_source = EncounterSourceNode.nodes.get_or_none(django_id=instance.encounter_source.id) if instance.encounter_source else None
...
def create_or_update_encounter_sim_center_node(sender, instance, created, **kwargs):
    ...
    patient = PatientNode.nodes.get_or_none(django_id=instance.patient.id) if instance.patient else None
    provider = ProviderNode.nodes.get_or_none(django_id=instance.provider.id) if instance.provider else None
    encounter_sim_center_nodes = EncounterSimCenterNode.nodes.filter(django_id=instance.id)
    if created:
    ...
    if encounter_source is not None:
        for encounter_sim_center_node in encounter_sim_center_nodes:
            ...
            encounter_sim_center_node.encounter_source.connect(encounter_source)
    if patient is not None:
        for encounter_sim_center_node in encounter_sim_center_nodes:
...
def delete_encounter_sim_center(sender, instance, **kwargs):
    try:
        encounter_sim_center = EncounterSimCenterNode.nodes.get(django_id=instance.id)
...
def create_or_update_encounter_rias_node(sender, instance, created, **kwargs):
    ...
    patient = PatientNode.nodes.get_or_none(django_id=instance.patient.id) if instance.patient else None
    provider = ProviderNode.nodes.get_or_none(django_id=instance.provider.id) if instance.provider else None
    encounter_rias_nodes = EncounterRIASNode.nodes.filter(django_id=instance.id)
    if created:
