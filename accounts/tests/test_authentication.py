...
class LogoutAPITest(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.db_manager("accounts").create_user(
            username="logoutuser", email="logout@example.com", password="testpass123"
        )
    ...
    def test_logout_with_valid_token(self):
        ...
        login_url = "/api/v1/accounts/auth/token/"
        login_data = {"username": "logoutuser", "password": "testpass123"}
        login_response = self.client.post(login_url, login_data, format="json")
...
class EmailLoginTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User.objects.db_manager("accounts").create_user(
            username="emailtestuser", email="emailtest@example.com", password="testpass123"
        )
        self.user2 = User.objects.db_manager("accounts").create_user(
            username="emailtestuser2", email="emailtest2@example.com", password="testpass123"
        )
    ...
    def test_login_with_username_still_works(self):
        ...
        url = "/api/v1/accounts/auth/token/"
        data = {"username": "emailtestuser2", "password": "testpass123"}
        response = self.client.post(url, data, format="json")
    ...
    def test_login_with_nonexistent_email(self):
        ...
        # May return 400 (validation error), 401 (unauthorized), or 403 (rate limited)
        self.assertIn(
    ...
    def test_login_with_email_wrong_password(self):
        ...
        url = "/api/v1/accounts/auth/token/"
        data = {"username": "emailtest@example.com", "password": "wrongpassword"}
        response = self.client.post(url, data, format="json")
...
class CookieAuthenticationTest(BaseTestCase):
    ...
    def setUp(self):
        """Set up test data and create fresh client."""
        super().setUp()
        # Create a fresh client to avoid CSRF token conflicts from previous tests
        self.client = APIClient()
