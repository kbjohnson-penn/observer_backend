...
class TierBasedAccessControlTest(APITestCase):
    ...
    def setUp(self):
        ...
        if hasattr(self.tier1_user, 'profile') and self.tier1_user.profile:
            ...
            self.tier1_profile = baker.make(
                Profile, user=self.tier1_user, tier=self.tier1, organization=self.organization, _using='accounts')
        ...
        if hasattr(self.tier2_user, 'profile') and self.tier2_user.profile:
            ...
            self.tier2_profile = baker.make(
                Profile, user=self.tier2_user, tier=self.tier2, organization=self.organization, _using='accounts')
        ...
        if hasattr(self.tier3_user, 'profile') and self.tier3_user.profile:
            ...
            self.tier3_profile = baker.make(
                Profile, user=self.tier3_user, tier=self.tier3, organization=self.organization, _using='accounts')
        ...
        self.visit1 = baker.make(VisitOccurrence, person=self.person1, provider=self.provider1, 
                               tier_id=self.tier1.id, _using='research')
        self.visit2 = baker.make(VisitOccurrence, person=self.person2, provider=self.provider2, 
                               tier_id=self.tier2.id, _using='research')
        self.visit3 = baker.make(VisitOccurrence, person=self.person3, provider=self.provider3, 
                               tier_id=self.tier3.id, _using='research')
    ...
    def test_tier1_user_cannot_access_higher_tier_detail(self):
        ...
        response = self.client.get(f'/api/v1/private/visits/{self.visit2.id}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        
        # Should get 404 for tier 3 person
        response = self.client.get(f'/api/v1/private/persons/{self.person3.id}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        
        # Should get 404 for tier 2 provider
        response = self.client.get(f'/api/v1/private/research-providers/{self.provider2.id}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_tier2_user_can_access_lower_tier_detail(self):
        ...
        response = self.client.get(f'/api/v1/private/visits/{self.visit1.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['id'], self.visit1.id)
        
        # Should access tier 1 person
        response = self.client.get(f'/api/v1/private/persons/{self.person1.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['id'], self.person1.id)
        
        # Should access tier 1 provider  
        response = self.client.get(f'/api/v1/private/research-providers/{self.provider1.id}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
