...
class EmptyResultsTest(APITestCase):
    ...
    def setUp(self):
        ...
        if hasattr(self.user, 'profile') and self.user.profile:
            ...
            self.profile = baker.make(Profile, user=self.user, tier=self.tier1, organization=self.organization, _using='accounts')
        ...
        self.visit = baker.make(VisitOccurrence, person=self.person, provider=self.provider, 
