...
from research.models import VisitOccurrence, Person, Provider
...
class FilterOptionsViewSet(BaseAuthenticatedViewSet):
    ...
    def list(self, request):
        ...
        accessible_providers = Provider.objects.using('research').filter(id__in=accessible_provider_ids)

        # Combine unique values from both patients and providers
        patient_genders = set(
            accessible_persons
            .values_list('gender_source_value', flat=True)
        ...
        patient_races = set(
            accessible_persons
            .values_list('race_source_value', flat=True)
        ...
        patient_ethnicities = set(
            accessible_persons
            .values_list('ethnicity_source_value', flat=True)
        ...
        person_yob = accessible_persons.aggregate(min=Min('year_of_birth'), max=Max('year_of_birth'))
        provider_yob = accessible_providers.aggregate(min=Min('year_of_birth'), max=Max('year_of_birth'))

        yob_min = min(
            person_yob['min'] or 9999,
            provider_yob['min'] or 9999
        ) if (person_yob['min'] or provider_yob['min']) else None

        yob_max = max(
            person_yob['max'] or 0,
            provider_yob['max'] or 0
        ) if (person_yob['max'] or provider_yob['max']) else None
        ...
        visit_date_range = accessible_visits.aggregate(
            earliest=Min('visit_start_date'),
            latest=Max('visit_start_date')
        )
        ...
        visit_options = {
            ...
            "visit_sources": list(
                accessible_visits
                .values_list('visit_source_value', flat=True)
                .distinct()
        ...
        clinical_options = {
            ...
            "notes": {
                "note_types": ["Progress Note", "Discharge Summary", "Consultation", "History and Physical"],
